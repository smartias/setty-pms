...
> **Remediation status (this branch):** all seven systemic failures have been addressed — #1 keyboard access (click-only controls converted to buttons, file inputs focusable, typeaheads and the SiteReport pin placement keyboard-operable), #2 focus visibility (`outline:none` removed, `:focus-visible` rings), #3 label association (~300 associations incl. the SettyPMS `Field` component), #4 modal semantics (dialog roles, Tab traps, Escape, focus restore across ~25 overlays), #5 live regions (toasts, status banners, validation alerts, aria-busy loading), #6 document structure (headings, landmarks, skip links), and #7 contrast — plus motion guards, table `scope`, Gantt/SVG naming, icon-button names, and color-only status pairing. Remaining known gaps: full `aria-activedescendant` combobox plumbing (roving-focus floor shipped instead), arbitrary-point keyboard pin placement in SiteReport (center-place + arrow nudge shipped), and AT/screen-reader runtime testing, which static review cannot replace.
