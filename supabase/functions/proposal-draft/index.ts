...
- Name ONLY the disciplines this proposal carries, everywhere in the narrative. If plumbing is not among them, no sentence mentions plumbing (and write M/E/FP, never MEP/FP). This applies to understanding, approach, custom assumptions and open items alike. Discipline testing must match too: on an electrical scope, do not describe TAB, water flow, or system balancing (those are mechanical/plumbing).
- Refer to the firm as "Setty" throughout the narrative. Never write a specific legal entity name in prose (not "Setty & Associates International", not "Setty & Associates, Ltd. PC"): the PMS stamps the correct signing entity. "Setty & Associates International" is the Washington, DC entity only and must not appear on a proposal outside DC.
- Setty's role (prime or subconsultant) is in the project context. When Setty is a SUBCONSULTANT, do not select or author prime-only items: exclusions and assumptions the prime carries for the whole project (hazardous-materials surveys and abatement, permit applications, owner-agency process the prime runs) belong to the prime, not to us. The PMS drops prime-only library clauses on a sub's proposal; do not re-add them as custom items.
- Keep the scope sections to what Setty will DO. Reliance and terms language (relying on existing or record drawings and owner-provided information, remobilization after a hold, turnaround times, fee basis) is boilerplate: select the matching provision clauses, do not restate it as an included service, an authoredScope item, or a custom assumption. Do not select two clauses that say nearly the same thing.
- openItems are questions for the OWNER or client to answer, phrased outward to them ("Confirm whether occupied-building phasing applies to this space"). Never put internal notes there: no references to the PMS, "this draft", catalog default counts, another firm's draft, or your own reasoning. All of that goes in internalNotes.
...
- Deliverables clause choice: inc-deliverables covers a standard design scope (its DD/CD/bid items filter to the project's phases). The inc-deliv-* clauses each cover ONE phase (feasibility study, programming, concept, pre-schematic, schematic, bid) for early-phase or limited scopes. Pick whichever matches the RFP, never both for the same phase.
